...
from `ivi_lua_core`. It builds the official Lua 5.4.9 source and `ivi_lua.c`
...
```powershell
cmake -S packages/ivi_lua_native/src -B build/native `
  -G "Visual Studio 17 2022" -A x64
cmake --build build/native --config Debug
$env:IVI_LUA_LIBRARY = (Resolve-Path build/native/Debug/ivi_lua_native.dll)
flutter test packages/ivi_lua_native
```

On Linux, from the repository root:

```bash
cmake -S packages/ivi_lua_native/src -B build/native -G Ninja
cmake --build build/native
export IVI_LUA_LIBRARY="$PWD/build/native/libivi_lua_native.so"
(cd packages/ivi_lua_native && flutter test)
```
...
configure with `-DIVI_LUA_SOURCE_DIR=/path/to/lua-5.4.9`.
