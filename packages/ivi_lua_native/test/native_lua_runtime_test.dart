...
void main() {
  ...
  final skipReason = libraryPath == null
      ? 'Build the pinned Lua shim first (see packages/ivi_lua_native/README.md).'
      : false;
...
String? _findNativeLibrary() {
  final override = Platform.environment['IVI_LUA_LIBRARY'];
  if (override != null && File(override).existsSync()) return override;
  final root = Directory.current.uri.resolve('../../');
  final candidates = Platform.isWindows
      ? [File.fromUri(root.resolve('build/native/Debug/ivi_lua_native.dll'))]
      : [
          File.fromUri(root.resolve('build/native/libivi_lua_native.so')),
          File.fromUri(root.resolve('build/native/Debug/libivi_lua_native.so')),
        ];
