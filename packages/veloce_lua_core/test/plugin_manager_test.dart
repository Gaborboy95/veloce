...
void main() {
  group('PluginManager', () {
    test(
      'loads, runs lifecycle, and cleans owned resources on unload',
      () async {
        ...
        final behavior = _Behavior(
          onLoad: (runtime, _) {
            ...
            runtime.callApi('events', 'subscribe', [
              const PluginApiDataArgument('demo.event'),
              PluginApiCallbackArgument(callback),
            ]);
            final token =
                runtime.callApi('ui', 'retain_callback', [
                      PluginApiCallbackArgument(callback),
                    ])!
                    as int;
            runtime.callApi('ui', 'register_tab', [
              PluginApiDataArgument({
                'id': 'demo',
                'title': 'Demo',
                'content': {'type': 'button', 'text': 'Run', 'callback': token},
              }),
            ]);
          },
        );
        ...
        expect(
          manager.pluginRegistry[source.manifest.id]!.state,
          PluginState.running,
        );
        ...
        expect(manager.pluginRegistry[source.manifest.id]!.generation, isNull);
      },
    );

    test(
      'failed transactional reload retains the running generation',
      () async {
        final root = await Directory.systemTemp.createTemp(
          'veloce-reload-test-',
        );
    ...
    test(
      'successful reload migrates structured state and drains old runtime',
      () async {
        final root = await Directory.systemTemp.createTemp(
          'veloce-state-test-',
        );
    ...
    test(
      'CAN API accepts wildcard and multiple-ID subscription filters',
      () async {
        final root = await Directory.systemTemp.createTemp(
          'veloce-can-api-test-',
        );
    ...
    test(
      'candidate API resources are rolled back when on_load fails',
      () async {
        final root = await Directory.systemTemp.createTemp(
          'veloce-rollback-test-',
        );
        final factory = _FakeRuntimeFactory();
    ...
    test(
      'permissions are enforced inside on_load without crashing manager',
      () async {
        final root = await Directory.systemTemp.createTemp(
          'veloce-permission-test-',
        );
        final factory = _FakeRuntimeFactory();
        factory.behaviors.add(
          _Behavior(
            onLoad: (runtime, _) => runtime.callApi('events', 'publish', const [
    ...
    test(
      'candidate permissions are generation-aware until reload commits',
      () async {
        final root = await Directory.systemTemp.createTemp(
          'veloce-grants-test-',
        );
        ...
        factory.behaviors.add(
          _Behavior(
            onLoad: (runtime, _) {
              ...
              runtime.callApi('events', 'publish', [
        ...
        await manager.loadPlugin(_source(root, permissions: const ['events']));
    ...
    test(
      'plugin storage distinguishes a stored null from a missing key',
      () async {
        final root = await Directory.systemTemp.createTemp(
          'veloce-null-store-test-',
        );
        var removed = false;
        ...
        final factory = _FakeRuntimeFactory()
          ..behaviors.add(
            _Behavior(
              onLoad: (runtime, _) {
                runtime.callApi('storage', 'set', const [
                  PluginApiDataArgument('nullable'),
                  PluginApiDataArgument(null),
                ]);
                removed =
                    runtime.callApi('storage', 'remove', const [
                          PluginApiDataArgument('nullable'),
                        ])!
                        as bool;
                absentAfterRemoval =
                    !(runtime.callApi('storage', 'contains', const [
                          PluginApiDataArgument('nullable'),
                        ])!
                        as bool);
              },
        ...
        await manager.loadPlugin(_source(root, permissions: const ['storage']));
    ...
    test('filesystem watcher hot-reloads a changed plugin once', () async {
      final root = await Directory.systemTemp.createTemp(
        'veloce-hot-reload-test-',
      );
      final pluginDirectory = await Directory.fromUri(
        root.uri.resolve('demo/'),
      ).create();
      await File.fromUri(
        pluginDirectory.uri.resolve('manifest.json'),
      ).writeAsString(
        jsonEncode({
...
PluginSource _source(Directory root, {List<String> permissions = const []}) {
  final effectivePermissions = [
    ...permissions,
    if (permissions.contains('ui.tabs') && !permissions.contains('ui.render'))
      'ui.render',
  ];
  final manifest = PluginManifestParser().parseMap({
    ...
    if (permissions.contains('can.read'))
      'can': {
        'read': [
          {'bus': 'comfort'},
          {
