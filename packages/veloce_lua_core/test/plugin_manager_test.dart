...
void main() {
  group('PluginManager', () {
    test('loads, runs lifecycle, and cleans owned resources on unload',
        () async {
      final root =
          await Directory.systemTemp.createTemp('veloce-manager-test-');
    ...
    test('failed transactional reload retains the running generation',
        () async {
      final root = await Directory.systemTemp.createTemp('veloce-reload-test-');
      final factory = _FakeRuntimeFactory();
    ...
    test('successful reload migrates structured state and drains old runtime',
        () async {
      final root = await Directory.systemTemp.createTemp('veloce-state-test-');
      final factory = _FakeRuntimeFactory();
    ...
    test('CAN API accepts wildcard and multiple-ID subscription filters',
        () async {
      final root =
          await Directory.systemTemp.createTemp('veloce-can-api-test-');
      final factory = _FakeRuntimeFactory();
      final behavior = _Behavior(
        onLoad: (runtime, _) {
    ...
    test('candidate API resources are rolled back when on_load fails',
        () async {
      final root =
          await Directory.systemTemp.createTemp('veloce-rollback-test-');
      final factory = _FakeRuntimeFactory();
    ...
    test('permissions are enforced inside on_load without crashing manager',
        () async {
      final root =
          await Directory.systemTemp.createTemp('veloce-permission-test-');
      final factory = _FakeRuntimeFactory();
      factory.behaviors.add(
        _Behavior(
    ...
    test('candidate permissions are generation-aware until reload commits',
        () async {
      final root = await Directory.systemTemp.createTemp('veloce-grants-test-');
      final factory = _FakeRuntimeFactory();
    ...
    test('plugin storage distinguishes a stored null from a missing key',
        () async {
      final root =
          await Directory.systemTemp.createTemp('veloce-null-store-test-');
      var removed = false;
    ...
    test('filesystem watcher hot-reloads a changed plugin once', () async {
      final root =
          await Directory.systemTemp.createTemp('veloce-hot-reload-test-');
