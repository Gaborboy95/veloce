...
final class DefaultNativeLuaLibraryResolver
  ...
  DynamicLibrary open() {
    ...
    if (Platform.isWindows) return DynamicLibrary.open('veloce_lua_native.dll');
    if (Platform.isLinux || Platform.isAndroid) {
      return DynamicLibrary.open('libveloce_lua_native.so');
    }
...
final class NativeLuaRuntimeFactory implements PluginScriptRuntimeFactory {
  ...
  Future<PluginScriptRuntime> create({
    ...
    final callable = NativeCallable<NativeHostCallback>.isolateLocal(
      (Pointer<Void> userData, VeloceLuaStatePointer state) =>
...
final class NativeLuaRuntime implements PluginScriptRuntime {
  ...
  late VeloceLuaStatePointer _state;
  ...
  Future<void> _initialize() async {
    ...
    _evaluate(
      luaApiBootstrap,
      '@veloce_api_v${manifest.apiVersion}.lua',
    ...
    for (final namespace in _apiRegistry.namespaces) {
      ...
      _evaluate(
        '_veloce_register_namespace(${jsonEncode(namespace.name)}, {$methods})',
        '@veloce_namespace_${namespace.name}.lua',
        PluginLifecyclePhase.loading,
      );
    }
    _evaluate(
      '_veloce_register_namespace = nil',
      '@veloce_namespace_finalize.lua',
      PluginLifecyclePhase.loading,
    );
  }
  ...
  int _dispatchHostCall(VeloceLuaStatePointer state) {
